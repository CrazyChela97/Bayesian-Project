...
#rural
giorni=1:364
basis <- create.fourier.basis(rangeval=c(1,364),nbasis=4)
...
ra_fit <- ggplot(data_rural,aes(x=Date, y=BC_trans, col=Station)) +
  geom_line() + 
  scale_color_manual(values=terrain.colors(50)[1:12]) +
  geom_line(data = mean_rural, aes(x=Date, y=rurale.fd.media), col = "darkred", size=1) +
  ylim(c(0,6)) +
  labs(title="PM10-Emilia: RURAL AREA") +
  theme(legend.position="none") 
...
usa_fit <- ggplot(data_us,aes(x=Date, y=BC_trans, col=Station)) +
  geom_line() + 
  scale_color_manual(values=terrain.colors(50)[13:50]) +
  geom_line(data = mean_us, aes(x=Date, y=sub_urb.fd.media), col = "darkred", size=1) +
  ylim(c(0,6)) +
  labs(title="PM10-Emilia: SUBURBAN & URBAN AREA") +
  theme(legend.position="none") 
...
# # # RURAL DATA # # #
trans_rural = trans_data[trans_data$Area == 'Rurale', ]
trans_rural$NomeStazione = as.factor(trans_rural$NomeStazione)
...
# ANALISI RESIDUI PER STAZIONE
rurale.fd.media.mat <- as.data.frame.matrix(matrix(data = apply(rurale.fd.media, 2, function(x) rep(x, dim(dati_per_stazione)[2])), ncol = dim(dati_per_stazione)[2]))
dati_per_stazione = dati_per_stazione - rurale.fd.media.mat
...
# # # URBAN & SUBURBAN DATA # # #
trans_us = trans_data[which(trans_data$Area == 'Urbano' | trans_data$Area == 'Suburbano'), ]
trans_us$NomeStazione = as.factor(trans_us$NomeStazione)
...
# Shapiro test + histogram
dati_BC= dati_per_stazione
shapiro_BC = rep(0,dim(dati_BC)[2]) #37
x11()
par(mfrow=c(2,5))
for (i in 1:10){
  shapiro_BC[i] = shapiro.test(dati_BC[,i])$p.value
...
# ANALISI RESIDUI PER STAZIONE
sub_urb.fd.media.mat <- as.data.frame.matrix(matrix(data = apply(sub_urb.fd.media, 2, function(x) rep(x, dim(dati_per_stazione)[2])), ncol = dim(dati_per_stazione)[2]))
dati_per_stazione = dati_per_stazione - sub_urb.fd.media.mat
